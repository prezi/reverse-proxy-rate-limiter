...
function RateLimiter(options) {
    ...
    this.IPResolver = new ipResolver.IPResolver(options.forwarded_headers);
...
RateLimiter.prototype = {
    ...
    initProxy: function () {
        ...
        this.sigtermEventHandler = function () {
            _this.terminate(0);
        };
        process.on('SIGTERM', this.sigtermEventHandler);

        this.messageEventHandler = function (message) {
            if (message === 'shutdown') {
                _this.terminate(0);
            }
        };
        process.on('message', this.messageEventHandler);

        this.uncaughtExceptionEventHandler = function(err) {
            logger.error('uncaughtException handler received: ' + err);
            _this.terminate(1);
        };
        process.on('uncaughtException', this.uncaughtExceptionEventHandler);
    },
    ...
    close: function (done) {
        ...
        this.server.close(function () {
            ...
            process.removeListener('SIGTERM', self.sigtermEventHandler);
            process.removeListener('message', self.messageEventHandler);
            process.removeListener('uncaughtException', self.uncaughtExceptionEventHandler);
            done();
        });
    },
    ...
    requestServed: function (proxyReq, req) {
