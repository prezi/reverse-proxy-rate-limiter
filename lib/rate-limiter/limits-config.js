...
LimitsConfigurationLoader.prototype = {
    ...
    parseConfig: function (callback) {
        if (this.limitsConfigurationSource.isUrl()) {
            this.loadConfigFromURL(this.configEndpoint, callback);
        } else if (this.limitsConfigurationSource.isFilePath()) {
            this.loadConfigFromFile(this.limitsConfigurationSource.parsedConfigEndpoint.path, callback);
        } else {
            throw new Error("Illegal url: " + this.configEndpoint);
        }
    },

    loadConfigFromURL: function (url, callback) {
        request({
            url: url,
            json: true
    ...
    loadConfigFromFile: function (path, callback) {
