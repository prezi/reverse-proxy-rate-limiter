...
describe("Integration tests", function () {
    ...
    it("should always allow /healthcheck/", function (done) {
        ...
        tester.sendRequest().onForwarded(function () {
            tester.sendRequest().onRejected(function () {
                tester.sendRequest(1, {"path": "healthcheck/"}).onForwarded(function () {
                    assert.equal(tester.rateLimiter.counter.getGlobal(), 1);
                    done();
                });
            });
    ...
    it("should not proxy requests to the configEndpoint", function (done) {
        ...
        tester.sendRequest(1, {"path": testEndpoint}).onRejected(function (res) {
            assert.equal(tester.pendingRequestsCount(), 0);
    ...
    it("should not fail if a 4xx response is served", function (done) {
        tester.sendRequest(1, {"expectedStatusCode": 401}).onForwarded(function () {
            assert.equal(tester.pendingRequestsCount(), 1);
    ...
    it("should not fail if a 5xx response is served", function (done) {
        tester.sendRequest(1, {"expectedStatusCode": 500}).onForwarded(function () {
            assert.equal(tester.pendingRequestsCount(), 1);
    ...
    var cfg2 = {
        ...
        buffer_ratio: 0.2,
        buckets: [
            {name: "default", limits: {capacity_unit: 4}},
            {name: "reuse", limits: {capacity_unit: 3}, conditions: [["header", "bucket", "eq", "reuse"]]},
            {name: "backup", limits: {capacity_unit: 1}, conditions: [["header", "bucket", "eq", "backup"]]}
        ]
    };
    var bucketReuse = {"bucket": "reuse"};
    var bucketBackup = {"bucket": "backup"};
    ...
    var maxRequestsPerIPConfig = {
        ...
        buffer_ratio: 0.2,
        buckets: [
            {name: "default", limits: {max_requests_per_ip: 2}}
        ]
    };
    var bucketDefault = {"bucket": "default"};
